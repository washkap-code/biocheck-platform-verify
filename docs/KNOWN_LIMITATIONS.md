...
- **verify-core is implemented but not hosted.** The FastAPI facade provides the face and fingerprint contracts and fails closed without its configured native sidecar. Its in-memory capture-ref store is suitable for one process; a multi-replica deployment requires a shared single-use store.
- **No independent penetration test** has been performed. Required before production.
- **KMS/HSM** is an adapter interface with a dev-only local implementation; a production adapter (AWS KMS / GCP KMS / on-prem HSM) must be configured. Production refuses to boot on the dev adapter.
- **Approved Concept 1 logo assets are present** in `public/brand/`; deployment smoke tests must still confirm they are served correctly.
